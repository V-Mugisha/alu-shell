...
# This script will create, make executable, and commit all required files
...
check_directory() {
    ...
    if [[ ! -d "$required_dir" ]]; then
        echo "Creating directory: $required_dir"
        mkdir -p "$required_dir"
    fi
    cd "$required_dir" || { echo "Failed to change to directory $required_dir"; exit 1; }
...
check_directory "processes_and_signals"

task6_content='#!/bin/bash
...
# Already in processes_and_signals directory

task7_content='#!/bin/bash
...
manage_my_process_content='#!/bin/bash
...
echo ""
echo "Created and committed the following files in processes_and_signals/:"
